...
class BouncyBall_Game(gym.Env):
    ...
    def step(self, action, playing=False, act=''):#每一步動作所要執行的內容
        self.done = False
        self.edge_judge()#判斷球有沒有到最左邊最右邊的邊界
        self.colliction_judge()#判斷球有沒有碰到平台
        self.get_score()#判斷分數
    ...
    def get_obs(self, ballpos, up_pos, down_pos):
        ...
        down_posy = (down_pos[1] + down_pos[3]) / 2 
